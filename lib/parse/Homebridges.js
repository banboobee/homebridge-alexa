...
function Homebridges(devices, context) {
  ...
  devices.forEach(function (element) {
    if (context.instanceBlacklist) {
      if (context.instanceBlacklist.find(x => x.toLowerCase() === element.instance.txt.id.toLowerCase())) {
	context.log(`Instance [${element.instance.txt.id}] on instance blacklist, ignoring.`);
	return;
      }
    }
