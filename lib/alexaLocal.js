...
function alexaLocal(options) {
  ...
  if (options.alexaService.contact) {
    Characteristic = options.Characteristic;
    options.alexaService.contact.setCharacteristic(Characteristic.ContactSensorState, Characteristic.ContactSensorState.CONTACT_NOT_DETECTED);
  }
  ...
  connection.client.on('connect', function () {
    ...
    if (options.alexaService.contact) {
      options.alexaService.contact.setCharacteristic(Characteristic.ContactSensorState, Characteristic.ContactSensorState.CONTACT_DETECTED);
    }
    ...
    connection.client.on('message', function (topic, message) {
      if (options.alexaService.listener) {
	options.alexaService.listener.setCharacteristic(Characteristic.On, true);
      }
      try {
        ...
        if (options.eventBus.listenerCount(msg.directive.header.namespace) > 0) {
          ...
          options.log('Emitting', JSON.stringify(msg, null, 2));
          options.eventBus.emit(msg.directive.header.namespace, msg, function (err, response) {
            ...
            options.log("Response", JSON.stringify(response, null, 2));
