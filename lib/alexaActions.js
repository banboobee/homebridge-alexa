...
function hapDiscovery(options) {
  ...
  homebridge.on('Ready', function () {
    alexaDiscovery.call(options, null, function (error, response) {
      // debug("Events", options);
      // options.log(JSON.stringify(response, null, 2));
      if (options.events) {
...
function checkDeviceList(endpoints) {
  if (this.deviceList && this.deviceList.length > 0 && ['allow', 'deny'].includes(this.deviceListHandling)) {
    ...
    if (allowed > 0) {
      this.log(`INFO: DeviceList - ${allowed} out of ${endpoints.length} devices are allowed.`);
    }
    if (denied > 0) {
      this.log(`INFO: DeviceList - ${denied} out of ${endpoints.length} devices are denied.`);
    }
    return (response);
  } else {
